...
*/
    ...
    file_list_old:=Split(Pipe("ls " * fld_old_code,"r"));
    
    tot:=#file_list_old; perc:=0; num_pol:=0; num_pp:=0;
    for i->file in file_list_old do
        if Truncate(i*100/tot) gt perc then perc+:=1; printf "%o%% %o %o\n",perc,num_pp,num_pol; end if;
        file_output:=fld_dest_pp * file;
        file_input:=fld_old_code * file;
        // if file_output exists then it should not contain any non-princ pols.
        // if so, we delete file_input otherwise we take only the non-princ lines from file_input and 
        // put them in file_output
        if OpenTest(file_output,"r") then
            printf "YES, file exists ... ";
            lines:=Split(Read(file_output));
            for l in lines do
                num_pol+:=1;
            ...
            Pipe("rm " cat file_input,"r");
            printf "done\n";
        else
            printf "NO, file does not exist ... ";
            lines:=Split(Read(file_input));
            for l in lines do
                num_pol+:=1;
                label:=Split(l,":")[1]; // label of form g.q.coeff-N.i.w.j-d.k
                split:=Split(label,"-.");
